...
from .utils import Retry, sign_dict, bv2av, encrypt_login_password, md5_bytes
...
def get_key(sid=None, jsessionid=None):
    ...
    post_data['sign'] = sign_dict(post_data, APPSECRET)
    cookie = {}
    if sid:
        cookie['sid'] = sid
    if jsessionid:
...
def login(username, password):
    ...
    encrypted_password = encrypt_login_password(password, hash, pubkey)
...
def login_by_access_token(access_token):
    ...
    login_params['sign'] = sign_dict(login_params, APPSECRET)
...
def upload_video_part(access_token, sid, mid, video_part: VideoPart, max_retry=5):
    ...
    with open(local_file_name, 'rb') as f:
        for chunk_id in range(0, chunk_total_num):
            ...
            status = Retry(max_retry=max_retry, check_return=True).run(
                upload_chunk,
                upload_url,
                server_file_name,
...
def upload(access_token,
    ...
    for video_part in parts:
        ...
        status = upload_video_part(access_token, sid, mid, video_part, max_retry)
        print("video part {} finished, status: {}".format(video_part.title, status))
        if not status:
            print("upload failed")
            return None, None
...
def edit_videos(
    ...
    for video_part in parts:
        print("upload {} added in pool".format(video_part.title))
        ret = upload_video_part(access_token, sid, mid, video_part, max_retry)
        print("video part {} finished, status: {}".format(video_part.title, ret))
        if not status:
            print("upload failed")
            return None, None
...
class BilibiliUploader:
    ...
    def login(self, username, password):
        code, self.access_token, self.refresh_token, self.sid, self.mid, _ = login(username, password)
        if code != 0: # success
    ...
    def upload(self, *args, **kws):
        return upload(self.access_token, self.sid, self.mid, *args, **kws)

    def edit(self, avid=None, bvid=None, parts=None, *args, **kws):
        ...
        edit_videos(
            self.access_token, self.sid, self.mid,
            avid, bvid, parts, *args, **kws)
